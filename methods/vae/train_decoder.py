...
parser.add_argument('--task', '-task', help='the task to train on. choose from [depth,segmentation,flow,prediction]', default='con2rgb', type=str)
...
if args.task == 'depth':
    ...
    train_ds, test_ds = import_data.load_dataset_for_training_decoder(args.dset_file, args.num_imgs, args.batch_size, 'flows')
elif args.task == 'con2rgb':
    train_ds, test_ds = import_data.load_dataset_for_training_decoder(args.dset_file, args.num_imgs, args.batch_size, 'rgbs')
else: #args.task == 'prediction'
    train_ds, test_ds = import_data.load_dataset_for_training_decoder(args.dset_file, args.num_imgs, args.batch_size, 'nexts')
...
if args.task == 'depth':
...
elif args.task == 'segmentation' or args.task == 'prediction':
    ...
    model = VAEModel(n_z=args.n_z, trainable_encoder=False, trainable_decoder=trainable_layers, out_channels=2)
    model.load_weights(args.model_path)
